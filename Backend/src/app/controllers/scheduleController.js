    ...
    async createSchedule(req, res) {
        try {
            ...
            if (createdSchedules.length === 0) {
                return res.status(400).json({ error: 'Tất cả các lịch trình đã tồn tại hoặc không hợp lệ.' });
            }
            res.status(201).json({ message: `Tạo ${createdSchedules.length} lịch trình thành công.`, schedules: createdSchedules });
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
    }
