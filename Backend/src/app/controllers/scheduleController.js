...
class ScheduleController {
  ...
  async getDoctorsBySpecialty(req, res) {
    try {
      ...
      if (!specialtyId)
        return res.status(400).json({ error: "Thiếu chuyên khoa." });
  ...
  async viewAllSchedules(req, res) {
    try {
      ...
      res
        ...
        .json({
          error:
            "Lỗi server khi lấy danh sách lịch trình. Vui lòng thử lại sau.",
        });
    }
  }
  ...
  async viewScheduleDetail(req, res) {
    try {
      ...
      res
        ...
        .json({
          error:
            "Lỗi server khi lấy chi tiết lịch trình. Vui lòng thử lại sau.",
        });
    }
  }
